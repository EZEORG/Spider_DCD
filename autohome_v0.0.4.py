...
def run(playwright):
    ...
    for index, card in enumerate(car_cards, start=1):
        try:
            ...
            if koubei_button:
                # 等待“口碑”按钮可见并可点击
                koubei_button.wait_for_element_state("visible")
                koubei_button.wait_for_element_state("stable")
                ...
                while True:
                    ...
                    for review_button in review_buttons:
                        try:
                            review_button.wait_for_element_state("visible")
                            review_button.wait_for_element_state("stable")
                            ...
                            if not csv_file_path:
                                ...
                                if current_fields != existing_fields:
                                    temp_file_path = Path(base_output_dir) / sanitize_filename(f'{car_name}_reviews_temp.csv')
                                    with open(csv_file_path, 'r', newline='', encoding='utf-8') as csv_file, \
                    ...
                    if page_count >= max_pages:
                        print(f"已达到最大翻页次数 {max_pages}")
                        break

                    next_button = koubei_page.query_selector('//a[contains(@class,"next")]')
                    if next_button:
                        next_button.wait_for_element_state("visible")
                        next_button.wait_for_element_state("stable")
                        next_button.click()
                        koubei_page.wait_for_load_state("networkidle")
                        time.sleep(2)
                    else:
