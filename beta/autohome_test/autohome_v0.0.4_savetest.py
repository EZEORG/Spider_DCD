                        ...
                        try:
                            ...
                            if not csv_file_path.exists():
                                ...
                                with open(csv_file_path, 'r', newline='', encoding='utf-8') as f:
                                    existing_fieldnames = f.readline().strip().split(',')
                                write_to_csv(csv_file_path, review_data, mode='a', fieldnames=existing_fieldnames)

                            review_progress[review_id] = {'reviewed': True}
                            save_progress(car_name_out, 'incomplete', last_user_id, review_progress)

                        except Exception as e:
                            logging.error(f"Review processing error: {e}")
                            take_screenshot(review_page, "review_error.png")

                except Exception as e:
                    logging.error(f"Error accessing review page: {e}")
                    take_screenshot(koubei_page, "koubei_page_error.png")

            save_progress(car_name_out, 'completed')

        except Exception as e:
            logging.error(f"Error processing car {car_name}: {e}")
            take_screenshot(page, "car_processing_error.png")
