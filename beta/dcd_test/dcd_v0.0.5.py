...
def run(playwright):
    ...
    while scroll_count < max_scrolls:
        ...
        for car_card in car_cards:
            car_info = car_card.inner_text()
            if car_info not in car_data:
                car_data.add(car_info)

        # 点击每个车的参数按钮并抓取数据
        for index, car_info in enumerate(car_data):
            try:
                # 重新获取当前页面所有车的卡片元素
                car_cards = page.query_selector_all('//div[contains(@class,"car-list_card")]')
                car_card = next((card for card in car_cards if card.inner_text() == car_info), None)
                ...
                if car_card:
                    ...
                    if param_button:
                        ...
                        for car_name in car_name_texts:
                            csv_file_path = Path(base_output_dir) / sanitize_filename(f'{car_name}.csv')
                            with open(csv_file_path, 'w', newline='', encoding='utf-8') as csv_file:
                                writer = csv.DictWriter(csv_file, fieldnames=['车名'] + attribute_names)
                                writer.writeheader()
                                row = {'车名': car_name}
                                row.update(car_data_dict[car_name])
                                writer.writerow(row)
                ...
                print(f"抓取 car {index + 1} 信息时出错：{e}")
